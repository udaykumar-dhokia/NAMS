...
import QRCodeModel from "./qrcode.model"; // your QR schema
import AttendanceModel from "../attendance/attendance.model"; // assuming you have a lecture/session schema
import crypto from "crypto";
import QRCode from "qrcode"; // for generating QR image
...
export const createQRCode = async (req: Request, res: Response) => {
    try {
        ...
        return res.status(201).json({
            ...
            qrId: newQr._id,
            token: newQr.token,
            qrImage, // base64 image for frontend to display
        });
    } catch (error) {
...
export const scanQRCode = async (req: Request, res: Response) => {
    try {
        const { token, studentId, lectureId } = req.body;
...
export const invalidateQRCode = async (req: Request, res: Response) => {
    try {
        ...
        return res.status(200).json({ message: "QR Code invalidated successfully" });
    } catch (err) {
...
export const getActiveQRCode = async (req: Request, res: Response) => {
    try {
        ...
        const qr = await QRCodeModel.findOne({ lecture: lectureId }).sort({ updatedAt: -1 });
        if (!qr) {
...
export const getQRHistory = async (req: Request, res: Response) => {
    try {
        ...
        const history = await QRCodeModel.find({ lecture: lectureId }).sort({ createdAt: -1 });
